...
Page({
  ...
  getDetail(id) {
    var that = this
    util.request("v2/rentservice/table_clean/get", {
      id,
    }, 'POST').then((res) => {
      that.setData({
        val: res
  ...
  cancel() {
    ...
    Dialog.alert({
      ...
      util.request("v2/rentservice/table_clean/save", {
        houseId: val.houseId,
        id: val.id,
        status: "4"
      }, 'POST').then((res) => {
  ...
  save() {
    ...
    if (active == 1) {
      ...
      util.request("v2/rentservice/table_clean/send_orders", {
        repairContentJjr,
        houseId: val.houseId,
        id: val.id,
      ...
      util.request("v2/rentservice/table_clean/my_handle_orders", {
        id: val.id,
        chengdanType,
        totalPrice,
        repairDetail
      ...
      util.request("v2/rentservice/table_clean/set_person_in_charge", {
        id: val.id, //订单id
  ...
  sure() {
    ...
    util.request("v2/rentservice/table_clean/finish_orders", {
      "status": 3,
      "id": val.id,
      "houseId": val.houseId,
  ...
  saveyan(){
    ...
    util.request("v2/rentservice/table_clean/check_orders", {
      id:val.id,        //保洁ID
